...
public:
    ...
    std::vector<std::vector<T>> Transponse() {
    ...
    friend Matrix<double> operator *(const T c, const Matrix<T> &a) {
        Matrix<T> res(a.Size());
    ...
    friend Matrix<double> operator -(const Matrix<T> &a, const Matrix<T> &b) {
        Matrix<T> res(a);
        for(int i = 0; i < a.Size(); ++i) {
            for (int j = 0; j < a.Size(); ++j) {
                res[i][j] -= b[i][j];
    ...
    friend std::vector<T> operator *(const std::vector<std::vector<T>> &a, const std::vector<std::vector<T>> &b) {
        std::vector<T> res(a.size());
        for (int i = 0; i < res.size(); ++i) {
            for (int j = 0; j < res.size(); ++j) {
                for (int k = 0; k < res.size(); ++k) {
